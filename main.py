...
async def periodic_sending():
    """Еженедельная отправка данных в канал с проверкой каждую минуту"""
    # Отмечаем, что отчет сегодня еще не отправлялся
    report_sent_today = False
    last_check_day = -1  # Для сброса флага при смене дня
    ...
    while True:
        try:
            ...
            # Сбрасываем флаг отправки при смене дня
            if current_day != last_check_day:
                report_sent_today = False
                last_check_day = current_day

            # Если сегодня день отправки, нужный час и минута, и отчет еще не отправлялся
            if (current_day == SEND_DAY_OF_WEEK and
                    current_hour == SEND_HOUR and
                    current_minute >= 40 and
                    not report_sent_today):
                logger.info(
                ...
                # Отмечаем, что отчет сегодня уже отправлялся
                report_sent_today = True
                logger.info("✅ Отчет отправлен, следующий отчет через неделю")

            # Каждый час выводим информацию о времени до следующей отправки
            if current_minute == 0:
                # Определяем, сколько дней осталось до дня отправки
                days_until_send_day = (SEND_DAY_OF_WEEK - current_day) % 7
                if days_until_send_day == 0 and (current_hour >= SEND_HOUR or report_sent_today):
                    days_until_send_day = 7  # Если сегодня день отправки, но время уже прошло/отчет отправлен

                # Определяем, сколько часов осталось до времени отправки
                hours_until_send = 0
                if days_until_send_day == 0:  # Если отправка сегодня
                    hours_until_send = SEND_HOUR - current_hour
                else:  # Если отправка в другой день
                    hours_until_send = (days_until_send_day - 1) * 24 + (24 - current_hour) + SEND_HOUR

                logger.info(
                    f"⏳ До следующей отправки отчета осталось {days_until_send_day} дней и {hours_until_send} часов")

            # Ждем ровно 1 минуту до следующей проверки
